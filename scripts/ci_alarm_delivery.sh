...
adb -s "$serial" shell am instrument -w -r \
  -e class com.timilehinaregbesola.mathalarm.AlarmResumeRecoveryTest,com.timilehinaregbesola.mathalarm.AlarmErrorPresentationTest \
...
grep -q 'OK (2 tests)' "$resume_report" || status=1
exit "$status"
